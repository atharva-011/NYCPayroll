...
d3.csv('https://raw.githubusercontent.com/atharva-011/NYCPayroll/main/Data/d3_data.csv').then(data => {
...
years.sort((a, b) => a - b);
    ...
    function updatePieChart(year) {
        ...
        const salaryByBorough = d3.rollup(filteredData,
                                          v => d3.sum(v, d => parseFloat(d['Daily.Salary'])),
                                          d => d['Work.Location.Borough']);
        const dataArray = Array.from(salaryByBorough, ([borough, totalSalary]) => ({borough, totalSalary}));
        ...
        texts.enter()
             .append("text")
             .merge(texts)
             .text(d => `${(d.data.totalSalary / totalSalaryAllBoroughs * 100).toFixed(1)}%`)
        ...
        paths.on('mouseover', (event, d) => {
            tooltip.transition()
                   .duration(200)
                   .style("opacity", .9);
            tooltip.html(`Borough: ${d.data.borough}<br>Total Salary: ${d.data.totalSalary.toFixed(2)}`)
